...
export const scans = {
  execute: (goal: string, target: string) => api.post('/hybrid/execute', { goal, target }),
  status: (id: string) => api.get(`/hybrid/status/${id}`),
  proposals: (id: string) => api.get(`/hybrid/proposals/${id}`),
  approve: (id: string, approved: string[]) => api.post(`/hybrid/approve/${id}`, { approved }),
...
export const system = {
  ...
  switchLLM: (provider: string, model?: string) => api.post('/system/llm/switch', { provider, model }),
  testLLM: () => api.get('/system/llm/test'),
  audit: (params?: string) => api.get(`/system/audit${params || ''}`),
  targets: () => api.get('/system/targets'),
};
