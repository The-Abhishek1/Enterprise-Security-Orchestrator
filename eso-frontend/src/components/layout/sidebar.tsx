...
const NAV = [
  ...
  { href: '/attack-surface', icon: '🛡', label: 'Attack Surface' },
        ...
        {NAV.map(n => {
          ...
          return (
            <Link key={n.href} href={n.href} onClick={() => setOpen(false)}
              className={clsx(
                'flex items-center gap-3 px-3 py-2 my-0.5 rounded-lg text-[13px] font-medium transition-all border border-transparent',
                active ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/15' : 'text-gray-400 hover:bg-white/[0.04] hover:text-gray-200'
              )}>
              <span className="text-sm w-5 text-center">{n.icon}</span>{n.label}
            </Link>
          );
        })}
      </nav>
