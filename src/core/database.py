...
class DatabaseManager:
    ...
    async def execute_postgres(self, query: str, *args) -> str:
        """Execute PostgreSQL query"""
        if not self.pg_pool:
            logger.warning("PostgreSQL not available, skipping query")
            return ""
    ...
    async def fetch_postgres(self, query: str, *args) -> list:
        ...
        if not self.pg_pool:
            logger.warning("PostgreSQL not available, skipping fetch")
            return []
        
        async with self.pg_pool.acquire() as conn:
            return await conn.fetch(query, *args)
    
    async def fetchrow_postgres(self, query: str, *args) -> Optional[asyncpg.Record]:
        """Fetch single row from PostgreSQL"""
        if not self.pg_pool:
            logger.warning("PostgreSQL not available, skipping fetchrow")
            return None
